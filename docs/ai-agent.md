...
- **Provider/model setup:** Settings → *AI Provider* writes `aiProvider` / `aiModel` / `aiApiKeys` (a per-provider `{ [provider]: key }` map, so switching providers never carries one provider's secret to another) / `aiBaseUrl`. `PiConfigWriter.writePiConfig()` (pure builders in `piConfig.ts`) materializes them into the agent dir before each spawn: `settings.json` (`defaultProvider`/`defaultModel`), `auth.json` (`{provider: {type:"api_key", key}}`, `0600`), and — when a custom base URL is set — a generated `registerProvider` override extension. Changing any of these keys disposes running Pi sessions (`onAiConfigChanged`) so the next `ai:start` respawns with fresh config.
