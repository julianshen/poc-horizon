...
describe("pageTranslator", () => {
  ...
  describe("translatePage", () => {
    it("calls executeJavaScript with extract script and returns nodes successfully", async () => {
      ...
      expect(executeJavaScriptMock).toHaveBeenNthCalledWith(
        1,
        expect.stringContaining("__horizonTranslate"),
        true,
      );
    ...
    it("batches nodes by character count (4000 char cap)", async () => {
      ...
      expect(translateBatch).toHaveBeenNthCalledWith(
        2,
        [text2, text3],
        "Spanish",
        { signal: undefined },
    ...
    it("returns { ok: false } when executeJavaScript throws on extraction", async () => {
      ...
      expect(res).toEqual({
        ok: false,
        error: "extract failed: Browser crashed",
      });
      expect(translateBatch).not.toHaveBeenCalled();
    });
    ...
    it("handles translateBatch returning all nulls", async () => {
      executeJavaScriptMock.mockResolvedValueOnce([{ id: 1, text: "Hello" }]);
    ...
    it("handles single node exceeding BATCH_CHAR_CAP", async () => {
      ...
      executeJavaScriptMock.mockResolvedValueOnce([{ id: 1, text: hugeText }]);
