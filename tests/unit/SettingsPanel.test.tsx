...
describe("SettingsPanel", () => {
  ...
  it("toggling a switch dispatches settings:set with the boolean", async () => {
    ...
    expect(api().invoke.mock.calls.some((c) => c[0] === "settings:set")).toBe(
      true,
    );
  });
  ...
  it("exposes the aiAdvertiseAgent toggle wired to settings:set", async () => {
    api().invoke.mockResolvedValue({
      ...
      aiConfirmActions: "never",
      proxyType: "system",
    });
    act(() => useBrowserStore.setState({ showSettings: true }));
    render(<SettingsPanel />);
    await waitFor(() =>
      expect(screen.getByText(/X-Horizon-Agent header/)).toBeTruthy(),
    );
    ...
    expect(api().invoke.mock.calls).toContainEqual([
      "settings:set",
      { key: "aiAdvertiseAgent", value: false },
    ]);
  });
  ...
  it.each(["direct", "system"] as const)(
    "hides rules/bypass inputs when proxyType is %s",
    async (type) => {
      api().invoke.mockResolvedValue({
        proxyType: type,
        proxyRules: "",
  ...
  it("proxy-rules input commits on BLUR (not on every keystroke) to avoid IPC thrash", async () => {
    ...
    expect(
      api().invoke.mock.calls.some(
        (c) =>
          c[0] === "settings:set" &&
          (c[1] as { key: string }).key === "proxyRules",
      ),
    ).toBe(false);
    ...
    expect(api().invoke.mock.calls).toContainEqual([
      "settings:set",
      { key: "proxyRules", value: "http=10.0.0.1:3128" },
    ]);
  });
  ...
  it("proxy-rules input commits on Enter and reverts on Escape", async () => {
    api().invoke.mockResolvedValue({
      proxyType: "manual",
      proxyRules: "http=old:8080",
      proxyBypassRules: "",
    });
    act(() => useBrowserStore.setState({ showSettings: true }));
    render(<SettingsPanel />);
    await waitFor(() => expect(screen.getByText("Proxy rules")).toBeTruthy());
    const rulesInput = screen.getByDisplayValue(
      "http=old:8080",
    ) as HTMLInputElement;
    // Edit + Escape → no IPC, draft reverts.
    await act(async () => {
      fireEvent.change(rulesInput, { target: { value: "http=garbage" } });
    });
    await act(async () => {
    ...
    await waitFor(() =>
      expect(
        (screen.getByPlaceholderText(/http=127/) as HTMLInputElement).value,
      ).toBe("http=old:8080"),
    );
    const rulesInput2 = screen.getByPlaceholderText(
      /http=127/,
    ) as HTMLInputElement;
    // Edit + Enter → one IPC commit.
    await act(async () => {
      fireEvent.change(rulesInput2, { target: { value: "http=new:9090" } });
    });
    await act(async () => {
      fireEvent.keyDown(rulesInput2, { key: "Enter" });
    });
    expect(api().invoke.mock.calls).toContainEqual([
      "settings:set",
      { key: "proxyRules", value: "http=new:9090" },
    ]);
  });
});
