...
describe('FindInPage', () => {
  ...
  it('Enter (no shift) dispatches find:start + find:next forward', () => {
    useBrowserStore.setState({ ...initialState, showFindBar: true, activeTabId: 't1' });
    render(<FindInPage />);
  ...
  it('Shift+Enter searches backwards', () => {
    ...
    expect(api().invokes[1]).toEqual({ channel: 'find:next', payload: { tabId: 't1', forward: false } });
  });
  ...
  it('search is a no-op when there is no active tab', () => {
    useBrowserStore.setState({ ...initialState, showFindBar: true, activeTabId: null });
    render(<FindInPage />);
    const input = screen.getByPlaceholderText('Find in page') as HTMLInputElement;
    fireEvent.change(input, { target: { value: 'q' } });
    fireEvent.keyDown(input, { key: 'Enter' });
