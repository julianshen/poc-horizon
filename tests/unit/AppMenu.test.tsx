...
describe('AppMenu', () => {
  ...
  it('lists the headline rows when open', () => {
    ...
    expect(screen.getByRole('menuitem', { name: /New Incognito Window/ })).toBeTruthy();
  ...
  it('New Tab row dispatches tab:create and closes the menu', () => {
    ...
    expect(api().invokes).toContainEqual({ channel: 'tab:create', payload: {} });
  ...
  it('every panel-toggle row toggles its store flag and closes the menu', () => {
    const rows: Array<{ name: RegExp | string; flag: 'showHistory' | 'showDownloads' | 'showSettings' }> = [
