...
describe('SkillsLibrary (bundled skills)', () => {
  ...
  beforeAll(() => {
    ...
    lib = new SkillsLibrary(path.join(__dirname, '../../resources/pi-extension/skills'));
  });
  ...
  it('readInteraction() rejects path traversal attempts', async () => {
    await expect(lib.readInteraction('../../SKILL.md')).rejects.toThrow(/invalid/);
    await expect(lib.readInteraction('no-extension')).rejects.toThrow(/invalid/);
  });
});
