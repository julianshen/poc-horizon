...
import {
  translatePage,
  restorePage,
} from "../../.electron/services/pageTranslator";
import { translateText } from "../../.electron/services/LlmTranslator";

const { registerIpcHandlers } =
  await import("../../.electron/ipc/main-handlers");
...
describe("IPC handlers", () => {
  describe("tabs", () => {
    it("tab:create dispatches createTab(url)", () => {
      ...
      expect(s.tabManager.createTab).toHaveBeenCalledWith(
        "https://example.com",
      );
    });
  ...
  describe("navigation", () => {
    it("navigation:go dispatches navigate(tabId, url)", () => {
      invoke(IPC_CHANNELS.NAVIGATION_GO, {
        tabId: "t1",
        url: "https://x.example",
      });
      expect(s.tabManager.navigate).toHaveBeenCalledWith(
        "t1",
        "https://x.example",
      );
    });
  ...
  describe("bookmarks", () => {
    ...
    it("bookmark:add forwards url+title+parentId", () => {
      invoke(IPC_CHANNELS.BOOKMARK_ADD, {
        url: "https://x",
        title: "X",
        parentId: "p",
      });
    ...
    it("bookmark:move forwards bookmarkId+parentId+index", () => {
      invoke(IPC_CHANNELS.BOOKMARK_MOVE, {
        bookmarkId: "b1",
        parentId: "p",
        index: 3,
      });
    ...
    it("bookmark:update forwards bookmarkId+changes", () => {
      invoke(IPC_CHANNELS.BOOKMARK_UPDATE, {
        bookmarkId: "b1",
        changes: { title: "New" },
      });
      expect(s.bookmarkManager.update).toHaveBeenCalledWith("b1", {
        title: "New",
      });
    });
  ...
  describe("downloads", () => {
    it.each([
      ...
      "%s forwards downloadId to downloadManager.%s",
      (channel, method) => {
  ...
  describe("find in page", () => {
    it("find:start calls view.webContents.findInPage(text, opts)", () => {
      invoke(IPC_CHANNELS.FIND_START, {
        tabId: "t1",
        text: "foo",
        caseSensitive: true,
      });
    ...
    it("find:start returns undefined when the tab has no view", () => {
      ...
      expect(
        invoke(IPC_CHANNELS.FIND_START, { tabId: "t1", text: "foo" }),
      ).toBeUndefined();
  ...
  describe("passwords", () => {
    ...
    it("password:remove forwards origin+username", () => {
      invoke(IPC_CHANNELS.PASSWORD_REMOVE, {
        origin: "https://x",
        username: "u",
      });
  ...
  describe("zoom + devtools + print", () => {
    ...
    it("print:toPDF forwards tabId+outputPath", async () => {
      await invoke(IPC_CHANNELS.PRINT_TO_PDF, {
        tabId: "t1",
        outputPath: "/tmp/out.pdf",
      });
      expect(s.tabManager.printToPDF).toHaveBeenCalledWith(
        "t1",
        "/tmp/out.pdf",
      );
    });
  ...
  describe("translation", () => {
    it("translate:page calls translatePage and returns result", async () => {
      vi.mocked(translatePage).mockResolvedValueOnce({
        ok: true,
        translated: 3,
        total: 3,
      });
      const res = await invoke(IPC_CHANNELS.TRANSLATE_PAGE, {
        targetLang: "Spanish",
      });
    ...
    it("translate:selection calls translateText and returns result", async () => {
      ...
      const res = await invoke(IPC_CHANNELS.TRANSLATE_SELECTION, {
        text: "Hello",
        targetLang: "Spanish",
      });
    ...
    it("translate:page stamps every progress event with the source tabId", async () => {
      vi.mocked(translatePage).mockImplementationOnce(
        async (_wc, _lang, onProgress) => {
          onProgress?.(5, 10);
      ...
      const sends = s.wcSend.mock.calls.filter(
    ...
    it("translate:cancel aborts the in-flight controller for the active tab", async () => {
      ...
      const inFlight = invoke(IPC_CHANNELS.TRANSLATE_PAGE, {
        targetLang: "Spanish",
      });
  ...
  describe("settings", () => {
    ...
    function wireStatefulSettings(initial: Record<string, unknown>): void {
      ...
      s.settingsManager.set.mockImplementation(
    ...
    it("settings:set for proxyType reads back the just-set value and applies to both sessions", async () => {
      ...
      // proxy apply too.
      await invoke(IPC_CHANNELS.SETTINGS_SET, {
        key: "proxyType",
        value: "manual",
      });
      const expected = {
    ...
    it("settings:set survives a setProxy rejection without throwing or leaking unhandled", async () => {
      wireStatefulSettings({
        proxyType: "manual",
        proxyRules: "malformed",
        proxyBypassRules: undefined,
      });
      defaultSetProxy.mockRejectedValueOnce(
        new Error("CHROMIUM_INVALID_PROXY_CONFIG"),
      );
