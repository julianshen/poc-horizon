...
function renderNode(id: string, ctx: RenderCtx): React.ReactNode {
  ...
  const next: RenderCtx = {
    surface: ctx.surface,
    seen: new Set(ctx.seen).add(id),
  };
  return <Component key={id} entry={entry} ctx={next} />;
}

const Component: React.FC<{ entry: ComponentEntry; ctx: RenderCtx }> = ({
  entry,
  ctx,
}) => {
...
function renderBody(body: ComponentBody, ctx: RenderCtx): React.ReactNode {
  ...
  if ("Heading" in body) {
    ...
    return (
      <TextNode text={text} hint={`h${body.Heading.level ?? 2}` as "h2"} />
    );
  }
  if ("Image" in body) {
    ...
    return (
      <div
        style={{
          ...
          flexDirection: "row",
          gap: body.Row.gap ?? 8,
          alignItems: alignOf(body.Row.align),
        }}
      >
    ...
    return (
      <div
        style={{
          ...
          gap: body.Column.gap ?? 8,
          alignItems: alignOf(body.Column.align),
        }}
      >
  ...
  if ("List" in body) {
    return (
      <ul style={{ margin: 0, paddingLeft: 18 }}>
        {body.List.children.map((cid) => (
          <li key={cid}>{renderNode(cid, ctx)}</li>
        ))}
      </ul>
    );
  }
...
const TextNode: React.FC<{
  text: string;
  hint?: "h1" | "h2" | "h3" | "h4" | "h5" | "caption" | "body";
}> = ({ text, hint = "body" }) => {
  if (hint === "caption")
    return (
      <span style={{ fontSize: 11, color: "var(--chrome-fg-muted)" }}>
        {text}
      </span>
    );
  ...
  return (
    <div style={{ fontSize: size[hint], fontWeight: 600, lineHeight: 1.2 }}>
      {text}
    </div>
  );
};
