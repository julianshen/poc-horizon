...
const READ_ONLY = new Set<string>([
  'screenshot', 'screenshotMarked', 'axtree', 'getDom', 'getUrl', 'getTitle',
  'describeAt',
  'listHelpers', 'cdpCollect',
  'domainSkillList', 'domainSkillRead', 'domainSkillSearch',
  'skillPreamble', 'skillListInteractions', 'skillReadInteraction',
  'tabList',
  'reader_extract',
]);
...
const META = new Set<string>([
  'workflowRecordStart', 'workflowRecordStop',
  'workflowRun', 'workflowList', 'workflowDelete',
]);
...
export class ActionRecorder {
  private active: { name: string; description?: string; steps: Step[] } | null = null;

  constructor(private readonly filePath: string) {}

  isRecording(): boolean { return this.active !== null; }
  activeName(): string | null { return this.active?.name ?? null; }

  start(name: string, description?: string): void {
    if (this.active) throw new Error(`already recording '${this.active.name}'; stop first`);
    if (!name || /[\\/]/.test(name)) throw new Error(`invalid workflow name: ${name}`);
...
export const RECORDABLE_TOOLS = (tool: string): boolean => !READ_ONLY.has(tool) && !META.has(tool);
