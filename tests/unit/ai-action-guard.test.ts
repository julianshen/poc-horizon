...
import { AiActionGuard, type ActionPolicy } from '@electron/services/AiActionGuard';
...
describe('HorizonBridgeServer + AiActionGuard integration', () => {
  it('a denied tool call returns ok:false with the denial reason', async () => {
    ...
    const { HorizonBridgeServer } = await import('@electron/services/HorizonBridgeServer');
    const fakeHarness = {
      click: vi.fn(async () => {}),
    } as never;
    const guard = new AiActionGuard(() => 'risky');
    guard.on('prompt', (p) => { guard.decide(p.id, false); });   // user denies
    const srv = new HorizonBridgeServer(fakeHarness, undefined, undefined, undefined, guard);
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      s.on('data', (chunk: string) => {
        ...
        if (nl === -1) return;
        const resp = JSON.parse(buf.slice(0, nl)) as { ok: boolean; error?: string };
        try {
      ...
      s.on('connect', () => s.write(JSON.stringify({ id: 'q', tool: 'click', args: { x: 1, y: 1 } }) + '\n'));
      s.on('error', reject);
    });
  });
});
