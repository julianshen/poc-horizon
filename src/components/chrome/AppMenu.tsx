...
export const AppMenu: React.FC<Props> = ({ open, onClose }) => {
  ...
  const open_ = (
    overlay: "showBookmarks" | "showHistory" | "showDownloads" | "showSettings",
  ) => {
  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-2 flex items-center gap-2">
        ...
        <MenuIconBtn
          aria-label="Zoom out"
          onClick={() => invokeOnTab("zoom:set", { level: ZOOM_OUT })}
        >
          −
        </MenuIconBtn>
        <MenuIconBtn
          aria-label="Reset zoom"
          onClick={() => invokeOnTab("zoom:reset")}
        >
          ↺
        </MenuIconBtn>
        <MenuIconBtn
          aria-label="Zoom in"
          onClick={() => invokeOnTab("zoom:set", { level: ZOOM_IN })}
        >
          +
        </MenuIconBtn>
      </div>
      ...
      <MenuRow
        label="Print…"
        hint="⌘P"
        onClick={() => invokeOnTab("print:start")}
      />
      <MenuRow
        label="Find in page"
        hint="⌘F"
      ...
      <MenuRow
        ...
        hint="⌘⌥I"
        onClick={() => invokeOnTab("devtools:toggle")}
      />
      <Divider />
      <MenuRow
        label="Settings"
        hint="⌘,"
        onClick={() => open_("showSettings")}
      />
    </div>
  );
};
...
const MenuIconBtn: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement>> = ({
  children,
  ...rest
}) => (
  <button
