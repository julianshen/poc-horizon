...
import { PasswordManager, type PasswordCrypto } from '@electron/services/PasswordManager';
...
const reverseCrypto: PasswordCrypto = {
  ...
  decrypt: (s) => Buffer.from(Buffer.from(s, 'base64')).reverse().toString('utf-8'),
};
...
describe('PasswordManager', () => {
  ...
  it('encrypts passwords on save and decrypts on load', () => {
    const pm = new PasswordManager(passwordsPath(), reverseCrypto);
    pm.saveEntry({ origin: 'https://example.com', username: 'alice', password: 'hunter2' });
  ...
  it('saveEntry() stamps createdAt on first save, lastUsedAt on overwrite', () => {
    const pm = new PasswordManager(passwordsPath(), reverseCrypto);
    pm.saveEntry({ origin: 'https://x.example', username: 'u', password: 'p1' });
    const first = pm.getAll()[0];
    expect(first.createdAt).toBeTypeOf('number');

    pm.saveEntry({ origin: 'https://x.example', username: 'u', password: 'p2' });
  ...
  it('saveEntry() treats (origin, username) as the composite key', () => {
    const pm = new PasswordManager(passwordsPath(), reverseCrypto);
    pm.saveEntry({ origin: 'https://x.example', username: 'alice', password: 'a' });
    pm.saveEntry({ origin: 'https://x.example', username: 'bob', password: 'b' });
    pm.saveEntry({ origin: 'https://y.example', username: 'alice', password: 'c' });
  ...
  it('getForOrigin() returns only entries for that origin', () => {
    const pm = new PasswordManager(passwordsPath(), reverseCrypto);
    pm.saveEntry({ origin: 'https://x.example', username: 'alice', password: 'a' });
    pm.saveEntry({ origin: 'https://y.example', username: 'alice', password: 'b' });
