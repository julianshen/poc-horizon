...
const tmp = useTmpDir("horizon-bookmarks");
const bookmarksPath = () => tmp.path("bookmarks.json");
...
describe("BookmarkManager", () => {
  ...
  describe("add / remove / update / move", () => {
    it("add() appends a bookmark with a generated id and persists", () => {
      ...
      expect(persisted.some((b: { id: string }) => b.id === added.id)).toBe(
        true,
      );
    });
    ...
    it("update() throws when the bookmark is missing", () => {
      const bm = new BookmarkManager(bookmarksPath());
      expect(() => bm.update("missing-id", { title: "x" })).toThrow(
        "Bookmark not found",
      );
    });
  ...
  describe("import / export", () => {
    ...
    it("export() round-trips through import()", () => {
      ...
      const bm2 = new BookmarkManager(tmp.path("bookmarks2.json"));
