...
describe("BrowserHarness", () => {
  ...
  it("scroll dispatches mouseWheel with deltas", async () => {
    ...
    expect(calls[0]).toEqual([
      "Input.dispatchMouseEvent",
      { type: "mouseWheel", x: 0, y: 0, deltaX: 0, deltaY: 400 },
    ]);
  });
  ...
  it("cdp() forwards method + params to webContents.debugger.sendCommand and returns the result", async () => {
    ...
    const cap = (
      wc.debugger.sendCommand as unknown as { mock: { calls: unknown[][] } }
    ).mock.calls;
  ...
  it("subscribeEvent + collectEvents buffers CDP events and drains them on demand", async () => {
    ...
    type Pvt = {
      cdpListener?: ((e: Electron.Event, m: string, p: unknown) => void) | null;
    };
    const listener = (h as unknown as Pvt).cdpListener!;
    listener({} as Electron.Event, "Network.responseReceived", {
      url: "https://a",
    });
    listener({} as Electron.Event, "Network.responseReceived", {
      url: "https://b",
  ...
  it("screenshotMarked returns PNG + marks and removes the overlay", async () => {
    ...
    const sendCommand = vi.fn(async (method: string) => {
      ...
      if (method === "Page.getLayoutMetrics")
        return { visualViewport: { clientWidth: 1200, clientHeight: 800 } };
      if (method === "Runtime.evaluate")
        return { result: { value: fakeMarks } };
      return {};
    });
    const { wc } = fakeWc({
      debugger: {
  ...
  it("screenshot accepts format + quality options", async () => {
    const sendCommand = vi.fn(async (method: string) => {
      ...
      if (method === "Page.getLayoutMetrics")
        return { visualViewport: { clientWidth: 100, clientHeight: 100 } };
      return {};
    });
    const { wc } = fakeWc({
      debugger: {
