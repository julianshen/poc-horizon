...
const sample = (id: string, overrides: Partial<TabType> = {}): TabType => ({
  id, schemaVersion: 1, url: `https://${id}`, title: id, isLoading: false, loadProgress: 0,
...
describe('Hover-handler coverage', () => {
  ...
  it('Tab: close button mouseenter/leave swaps colours', () => {
    const { container } = render(<Tab tab={sample('a')} isActive={false} index={0} />);
    const close = container.querySelector('[data-testid="tab-close"]') as HTMLElement;
    fireEvent.mouseEnter(close);
    fireEvent.mouseLeave(close);
  });

  it('TabBar: new-tab button mouseenter/leave', () => {
    act(() => useBrowserStore.setState({ tabs: [sample('a')], activeTabId: 'a' }));
    render(<TabBar />);
  ...
  it('BookmarksBar: pill mouseenter/leave', async () => {
    api().invoke.mockResolvedValue([
      { id: 'a', schemaVersion: 1, index: 0, title: 'Alpha', url: 'https://a', dateAdded: 0 },
    ]);
    render(<BookmarksBar />);
  ...
  it('BookmarksPanel: row hover + Remove button click', async () => {
    ...
    expect(api().invoke.mock.calls.some((c) => c[0] === 'bookmark:remove')).toBe(true);
  });

  it('HistoryPanel: row hover + Clear all', async () => {
    api().invoke.mockResolvedValue([
      { id: 'a', url: 'https://a', title: 'Alpha', visitTime: Date.now(), visitCount: 1, typedCount: 0 },
    ]);
    ...
    expect(api().invoke.mock.calls.some((c) => c[0] === 'history:clear')).toBe(true);
  });
});
