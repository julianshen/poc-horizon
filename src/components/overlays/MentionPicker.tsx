 ...
 */
export const MentionPicker: React.FC<Props> = ({ alreadyPicked, onPick, onClose }) => {
  ...
  const pick = useCallback((tabId: string, title: string) => {
    onPick(tabId, title);
    onClose();
  }, [onPick, onClose]);
  ...
  return (
    ...
    >
      <input
        ...
        style={{ padding: '10px 14px', borderBottom: '0.5px solid var(--chrome-border)', color: 'var(--chrome-fg)' }}
      />
      <div className="overflow-y-auto" style={{ maxHeight: 220 }}>
        {filtered.length === 0 && (
          <div className="text-xs px-4 py-3" style={{ color: 'var(--chrome-fg-subtle)' }}>No matching tabs</div>
        )}
        {filtered.map((t, i) => {
          ...
          return (
            <button
              ...
              style={{
                background: i === highlight ? 'var(--tab-bg-hover)' : 'transparent',
            ...
            >
              {t.favicon ? (
                <img src={t.favicon} alt="" className="w-3.5 h-3.5 shrink-0 rounded-[4px]" />
              ) : (
                <span
              ...
              {already && <span className="text-[10px]" style={{ color: 'var(--chrome-fg-subtle)' }}>added</span>}
            </button>
          );
        })}
      </div>
