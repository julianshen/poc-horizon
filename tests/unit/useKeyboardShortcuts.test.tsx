...
describe('useKeyboardShortcuts', () => {
  ...
  it('Cmd/Ctrl+Shift+N invokes window:newIncognito', () => {
    renderHook(() => useKeyboardShortcuts());
    const ev = new KeyboardEvent('keydown', { key: 'N', metaKey: true, shiftKey: true, cancelable: true });
    window.dispatchEvent(ev);
    expect(api().invokes).toContainEqual({ channel: 'window:newIncognito', payload: {} });
  });
  ...
  it('Cmd/Ctrl+P invokes print:start for the active tab', () => {
    ...
    renderHook(() => useKeyboardShortcuts());
    press('p');
    expect(api().invokes).toContainEqual({ channel: 'print:start', payload: { tabId: 'a' } });
  });

  it('Cmd/Ctrl+Shift+I invokes devtools:toggle for the active tab', () => {
    useBrowserStore.setState({ ...initialState, activeTabId: 'a' });
    renderHook(() => useKeyboardShortcuts());
    const ev = new KeyboardEvent('keydown', { key: 'I', metaKey: true, shiftKey: true, cancelable: true });
    window.dispatchEvent(ev);
    expect(api().invokes).toContainEqual({ channel: 'devtools:toggle', payload: { tabId: 'a' } });
  });
  ...
  it('Cmd/Ctrl+= and + zoom in to 1.2x', () => {
    ...
    renderHook(() => useKeyboardShortcuts());
    press('=');
    expect(api().invokes).toContainEqual({ channel: 'zoom:set', payload: { tabId: 'a', level: 1.2 } });
  });
  ...
  it('Cmd/Ctrl+- zooms out to 0.9x', () => {
    ...
    renderHook(() => useKeyboardShortcuts());
    press('-');
    expect(api().invokes).toContainEqual({ channel: 'zoom:set', payload: { tabId: 'a', level: 0.9 } });
  });
  ...
  it('Cmd/Ctrl+0 resets zoom on the active tab', () => {
    ...
    renderHook(() => useKeyboardShortcuts());
    press('0');
    expect(api().invokes).toContainEqual({ channel: 'zoom:reset', payload: { tabId: 'a' } });
  });
