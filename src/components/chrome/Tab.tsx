...
export const Tab: React.FC<TabProps> = ({ tab, isActive, index }) => {
  ...
  const group = useBrowserStore((s) =>
    s.groups.find((g) => g.id === tab.groupId),
  );
  ...
  const toggleMute = useCallback(
    (e: React.MouseEvent) => {
      e.stopPropagation();
      window.horizonAPI.invoke(tab.isMuted ? "tab:unmute" : "tab:mute", {
        tabId: tab.id,
      });
    },
    [tab.id, tab.isMuted],
  ...
  const width = tab.isPinned
    ? "w-9 min-w-[36px] max-w-[36px]"
  ...
  return (
    <>
      ...
      >
        ...
        {tab.isLoading ? (
          ...
          <img
            src={tab.favicon}
            alt=""
            className="w-[14px] h-[14px] shrink-0 rounded-[4px]"
          />
        ) : (
          <span
      ...
      {menu && (
        <TabContextMenu tab={tab} x={menu.x} y={menu.y} onClose={closeMenu} />
      )}
    </>
  );
};
