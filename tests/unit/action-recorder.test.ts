...
describe('ActionRecorder', () => {
  ...
  it('captures side-effecting tool calls between start and stop', async () => {
    ...
    expect(wf.steps[0]).toEqual({ tool: 'navigate', args: { url: 'https://a.com/orders' } });
  });
  ...
  it('overwrites by name on stop', async () => {
    r.start('w'); r.capture('click', { x: 1, y: 1 }); await r.stop();
    r.start('w'); r.capture('click', { x: 2, y: 2 }); r.capture('type', { text: 'hi' }); await r.stop();
    const wf = r.get('w');
  ...
  it('list() returns workflow names without step bodies (cheap)', async () => {
    r.start('a'); r.capture('navigate', { url: 'x' }); await r.stop();
    r.start('b'); r.capture('navigate', { url: 'y' }); await r.stop();
    const ls = r.list();
  ...
  it('get() returns the full workflow including steps', async () => {
    r.start('w'); r.capture('click', { x: 5, y: 5 }); await r.stop();
  ...
  it('cancel() drops the in-progress recording without persisting', async () => {
    r.start('w'); r.capture('click', { x: 1, y: 1 });
    r.cancel();
    expect(r.isRecording()).toBe(false);
    expect(r.list()).toEqual([]);
  });
