...
function makeResolver(impl: (url: string, headers: Record<string, string>) => Promise<{ status: number; body: string; etag: string | null }>): AgentPolicyResolver {
  return new AgentPolicyResolver(impl);
}

describe('AgentPolicyResolver.originOf', () => {
  it('returns the origin for http(s) URLs', () => {
    expect(AgentPolicyResolver.originOf('https://example.com/path?q=1')).toBe('https://example.com');
  });
...
describe('AgentPolicyResolver.resolve', () => {
  it('fetches /agent.json from the origin root and caches the result', async () => {
    const fetcher = vi.fn(async () => ({ status: 200, body: validPolicy, etag: '"a"' }));
  ...
  it('sends If-None-Match on revalidation and reuses cached policy on 304', async () => {
    let calls = 0;
    const fetcher = vi.fn(async (_url: string, headers: Record<string, string>) => {
      calls++;
      if (calls === 1) return { status: 200, body: validPolicy, etag: '"v1"' };
    ...
    (r as unknown as { cache: Map<string, { fetchedAt: number }> }).cache.get('https://x.test')!.fetchedAt = 0;
  ...
  it('returns null for invalid JSON', async () => {
    const fetcher = vi.fn(async () => ({ status: 200, body: 'not json', etag: null }));
  ...
  it('returns null for valid JSON that fails the v1 shape check', async () => {
    const fetcher = vi.fn(async () => ({ status: 200, body: JSON.stringify({ foo: 'bar' }), etag: null }));
    expect(await makeResolver(fetcher).resolve('https://x.test')).toBeNull();
  });
  ...
  it('cached() returns whatever is in the cache without fetching', async () => {
    const fetcher = vi.fn(async () => ({ status: 200, body: validPolicy, etag: null }));
    const r = makeResolver(fetcher);
