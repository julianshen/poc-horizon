...
describe('parseLlmsTxt', () => {
  it('extracts title, summary, sections and links', () => {
    ...
    expect(parsed.summary).toBe('A modern, fast web framework for building APIs with Python.');
    expect(parsed.sections).toHaveLength(2);
    expect(parsed.sections[0]).toEqual({
      name: 'Getting Started',
      links: [
        { title: 'Installation', url: 'https://fastapi.tiangolo.com/#installation', description: 'pip install fastapi' },
