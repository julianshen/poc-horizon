...
describe("DomainSkills", () => {
  ...
  it("rejects file names with path separators", async () => {
    await expect(ds.save("s.com", "../escape.md", "x")).rejects.toThrow(
      /invalid skill name/,
    );
    await expect(ds.read("s.com", "foo/bar.md")).rejects.toThrow(
      /invalid skill name/,
    );
  });

  it("rejects hosts with path separators", async () => {
    expect(() => DomainSkills.normalizeHost("bad/host")).toThrow(
      /invalid host/,
    );
  });
