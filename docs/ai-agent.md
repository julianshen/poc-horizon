...
- **Build:** `npm run build:pi` (in `scripts/build-pi.mjs`) uses `bun build --compile` to pack `@earendil-works/pi-coding-agent` into a single executable at `resources/bin/pi` (git-ignored), copying the assets Pi loads relative to the binary (`package.json`, `theme/`, `assets/`, `export-html/`, `docs/`, photon WASM). Accepts `--target=bun-<os>-<arch>` (cross-compile) and `--out=<dir>`.
- **Per-arch packaging:** the `dist*` scripts run `build:pi` once for the host arch (so `extraResources` has something to copy), then the electron-builder `afterPack` hook (`scripts/afterPack-pi.cjs`) rebuilds Pi with the matching `bun --target` for **each** packed platform/arch and overwrites that app's `Resources/bin` — so a multi-arch `dist:mac` (x64 + arm64) ships the correct binary in each artifact rather than the host one in both.
...
- **Provider/model setup:** Settings → *AI Provider* writes `aiProvider` plus per-provider `{ [provider]: value }` maps — `aiModels` / `aiApiKeys` / `aiBaseUrls` — so switching providers never carries one provider's model, secret, or endpoint over to another. `PiConfigWriter` materializes **all** configured providers so they co-exist in Pi: `auth.json` gets an `api_key` for every keyed provider (Pi gates `/model` / `cycle_model` on auth presence), base-URL overrides are registered for each in the generated extension, and `enabledModels` is set to the authed providers' models so Pi's `cycle_model` rotates over exactly the user's set. `aiProvider` is just Pi's `defaultProvider` (the model the session opens on). A secrets-free sidecar (`.horizon-managed-auth.json`) records which keys Horizon wrote so it only ever prunes its own — Pi `/login` / OAuth credentials are preserved. Runtime switching via the RPC `set_model`/`cycle_model` commands is Phase 2 (see [pi-multiprovider-model-switching.md](pi-multiprovider-model-switching.md)). `PiConfigWriter.writePiConfig()` (pure builders in `piConfig.ts`) materializes them into the agent dir before each spawn: `settings.json` (`defaultProvider`/`defaultModel`), `auth.json` (`{provider: {type:"api_key", key}}`, `0600`), and — when a custom base URL is set — a generated `registerProvider` override extension. Changing any of these keys disposes running Pi sessions (`onAiConfigChanged`) so the next `ai:start` respawns with fresh config.
