...
test.beforeAll(async () => {
  ...
  app = await electron.launch({
    ...
    env: {
      ...process.env,
      HORIZON_DISABLE_RESTORE: "1",
      HORIZON_DISABLE_AI_SPAWN: "1",
    },
    timeout: 20_000,
  });
  ...
  await app.evaluate(async ({ BrowserWindow }) => {
    ...
    if (v) {
      await v.webContents.loadURL(
        'data:text/html,<html><body style="margin:0;background:%23d04545;color:white;font:48px sans-serif"><div style="padding:40px">PAGE CONTENT — should be hidden behind menus</div></body></html>',
      );
    }
  });
...
test("collect diagnostics", async () => {
  ...
  await win.screenshot({
    path: "./test-results/diag-default.png",
    fullPage: false,
  });
  ...
  const backDisabled = await win
    .getByRole("button", { name: "Back" })
    .isDisabled();
  const fwdDisabled = await win
    .getByRole("button", { name: "Forward" })
    .isDisabled();
  log(
    `Back disabled = ${backDisabled} (expect false), Forward disabled = ${fwdDisabled} (expect true)`,
  );
  ...
  const fwdEnabledAfterBack = !(await win
    .getByRole("button", { name: "Forward" })
    .isDisabled());
  ...
  await win.evaluate(() => {
    (
      window as unknown as { __navStateEvents: Array<{ url: string }> }
    ).__navStateEvents = [];
    window.horizonAPI.on("navigation:state", (s: unknown) => {
      (
        window as unknown as { __navStateEvents: Array<{ url: string }> }
      ).__navStateEvents.push(s as { url: string });
    });
  });
  await app.evaluate(async ({ BrowserWindow }) => {
  ...
  const sawSpaNavState = await win.evaluate(() =>
    (
      window as unknown as { __navStateEvents: Array<{ url: string }> }
    ).__navStateEvents.some((e) => e.url.includes("spa-route-1")),
  );
  log(
    `navigation:state fired for pushState URL: ${sawSpaNavState} (expect true)`,
  );
  expect(
    sawSpaNavState,
    "pushState navigation should produce a navigation:state IPC",
  ).toBe(true);
  void navStateEvents;
  ...
  const listenerCount = await app.evaluate(({ BrowserWindow }) => {
    const wc =
      BrowserWindow.getAllWindows()[0]!.getBrowserViews()[0]!.webContents;
    return wc.listenerCount("context-menu");
  });
  log(
    `Web context-menu listeners on tab webContents: ${listenerCount} (expect >= 1)`,
  );
  expect(
    listenerCount,
    "TabManager should register a context-menu listener",
  ...
  if (!piAvailable) {
    ...
    await win.evaluate(() =>
      window.horizonAPI.invoke("ai:start", {
        prompt: 'Say "ok" exactly and nothing else.',
      }),
    );
    ...
    await win.waitForFunction(
      () =>
        (
          window as unknown as { __aiEvents: { type: string }[] }
        ).__aiEvents.some((e) => e.type === "turn_end"),
      { timeout: 30_000 },
    );
    const events = await win.evaluate(
      () =>
        (window as unknown as { __aiEvents: { type: string }[] }).__aiEvents,
    );
    const types = events.map((e) => e.type);
    log(`Pi agent emitted event types: ${JSON.stringify(types)}`);
    expect(types, "Pi should stream at least one text_delta").toContain(
      "text_delta",
    );
  ...
  log(
    `Session spell-checker languages: ${JSON.stringify(spellInfo.sessionLanguages)} (expect non-empty)`,
  );
  expect(
    spellInfo.sessionLanguages.length,
    "session.getSpellCheckerLanguages should be set from settings",
  ).toBeGreaterThan(0);
  ...
  await win.evaluate(
    (id) =>
      window.horizonAPI.invoke("devtools:toggle", { tabId: id as string }),
    tabId!,
  );
  ...
  const devToolsOpen = await app.evaluate(({ BrowserWindow, webContents }) => {
    ...
    const wc = w.getBrowserViews()[0]!.webContents;
    return {
      open: wc.isDevToolsOpened(),
      webContentsCount: webContents.getAllWebContents().length,
    };
  });
  log(
    `DevTools open after devtools:toggle: ${devToolsOpen.open} (expect true), webContents total = ${devToolsOpen.webContentsCount}`,
  );
  expect(
    devToolsOpen.open,
    "devtools:toggle IPC should open DevTools on the active tab",
  ).toBe(true);
  ...
  if ((await menuBtn.count()) > 0) {
    ...
    await win.screenshot({
      path: "./test-results/diag-appmenu.png",
      fullPage: false,
    });
  ...
  await win.screenshot({
    path: "./test-results/diag-cmdk.png",
    fullPage: false,
  });
  ...
  if ((await tab.count()) > 0) {
    ...
    await win.screenshot({
      path: "./test-results/diag-tabmenu.png",
      fullPage: false,
    });
