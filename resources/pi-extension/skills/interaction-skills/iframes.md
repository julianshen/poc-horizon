...
Drop into iframe-aware code only when you need to *read* DOM content inside the frame.
...
For most cases, prefer to *navigate to the iframe URL directly* — it bypasses the parent entirely. Read the `src` attribute, navigate the top-level tab to it, do your work, navigate back.

## Coordinates: page vs frame

Click coordinates are always in *page* space (top-left of the visible viewport = 0,0). Don't add the iframe's offset twice — the hit test does it for you.
