...
const emit = (api: ReturnType<typeof setupRendererTest>['api'], p: { id: string; tool: string; summary: string; args?: Record<string, unknown> }): void => {
...
describe('AiActionPrompt', () => {
  ...
  it('shows the tool name and the summary on prompt', () => {
    render(<AiActionPrompt />);
    act(() => emit(api, { id: 'a1', tool: 'click', summary: 'Click at (100, 50)' }));
    expect(screen.getByRole('dialog', { name: 'Agent action approval' })).toBeTruthy();
  ...
  it('Allow sends ai:actionDecide { allow: true } and clears the prompt', () => {
    render(<AiActionPrompt />);
    act(() => emit(api, { id: 'a2', tool: 'navigate', summary: 'Navigate to https://x' }));
  ...
  it('Block sends ai:actionDecide { allow: false } and clears the prompt', () => {
    render(<AiActionPrompt />);
    act(() => emit(api, { id: 'a3', tool: 'evaluate', summary: 'Run JS: window.x = 1' }));
  ...
  it('queues a second prompt and shows "+N more" while the first is open', () => {
    render(<AiActionPrompt />);
    act(() => emit(api, { id: 'q1', tool: 'click', summary: 'Click at (1, 1)' }));
