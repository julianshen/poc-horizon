...
const tab = (id: string, title = id): Tab => ({
  id, schemaVersion: 1, url: `https://${id}`, title, isLoading: false, loadProgress: 0,
...
describe('useCommandItems', () => {
  ...
  it('always includes the sticky Ask Horizon row, even with no match', () => {
    const { result } = renderHook(() => useCommandItems('asdfqwertynevermatch', () => {}));
    expect(result.current[0].kind).toBe('ai');
    expect(result.current.every((it) => it.kind === 'ai' || it.label.toLowerCase().includes('asdfqwertynevermatch'))).toBe(true);
  });
  ...
  it('panel rows invoke toggleOverlay and close', () => {
    ...
    const bookmarks = result.current.find((it) => it.label === 'Open Bookmarks');
  ...
  it('Reload current tab dispatches navigation:reload when there is an active tab', () => {
    ...
    const { result } = renderHook(() => useCommandItems('', () => {}));
    const reload = result.current.find((it) => it.label === 'Reload current tab');
    act(() => reload!.action());
    expect(api().invokes).toContainEqual({ channel: 'navigation:reload', payload: { tabId: 'x' } });
  });
});
