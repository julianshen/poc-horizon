...
describe("applyA2UIMessage", () => {
  ...
  it("surfaceUpdate merges components by id; latest write wins", () => {
    let s: SurfaceState | null = applyA2UIMessage(null, {
  ...
  it("dataModelUpdate sets a path value", () => {
    let s = applyA2UIMessage(null, {
      beginRendering: { surfaceId: "s1", root: "r" },
    });
    s = applyA2UIMessage(s, {
      dataModelUpdate: { surfaceId: "s1", path: "/title", contents: "Hello" },
    });
  ...
  it("deleteSurface returns null when ids match, preserves state when they don't", () => {
    ...
    expect(
      applyA2UIMessage(s, { deleteSurface: { surfaceId: "s1" } }),
    ).toBeNull();
    expect(applyA2UIMessage(s, { deleteSurface: { surfaceId: "other" } })).toBe(
      s,
    );
  });
  ...
  it("messages targeting a different surfaceId are ignored", () => {
    ...
    const after = applyA2UIMessage(s, {
      dataModelUpdate: { surfaceId: "other", path: "/x", contents: 1 },
    });
    expect(after).toBe(s);
  });
});
...
describe("resolveValue", () => {
  ...
  it("returns the path value from the data model", () => {
    expect(resolveValue({ path: "/title" }, { "/title": "Hello" })).toBe(
      "Hello",
    );
  });
