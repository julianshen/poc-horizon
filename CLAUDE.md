...
- **Agent Policy v1 (site `/agent.json`)** layers on top of the user's `aiConfirmActions`. The guard's `evaluate()` returns `allow` / `prompt` / `deny`; bridge dispatch treats `deny` as a hard fail with the spec-trigger as reason (no user prompt). `AgentPolicyResolver` is per-origin with ETag + 24h TTL. The active policy is refreshed on every navigate. Full status: [`docs/agent-policy-status.md`](docs/agent-policy-status.md).
