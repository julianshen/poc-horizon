...
describe('useDownloads', () => {
  ...
  it('upserts download:created and download:updated events', () => {
    ...
    act(() => api().emit('download:updated', { ...item('a'), receivedBytes: 75 }));
  ...
  it('upserts download:completed and download:failed events', () => {
    ...
    act(() => api().emit('download:failed', { ...item('b'), state: 'interrupted' as const }));
    expect(result.current.downloads.find((d) => d.id === 'b')?.state).toBe('interrupted');
  });
  ...
  it('clearCompleted invokes IPC and removes non-progressing items locally', () => {
    ...
    expect(api().invokes.some((i) => i.channel === 'download:clearCompleted')).toBe(true);
  });
});
