...
test("Cmd+K opens the command palette and Escape closes it", async () => {
  ...
  await expect(
    win.getByPlaceholder("Ask Horizon, or type a command…"),
  ).toBeVisible();
...
test("AI toggle button opens and closes the sidebar", async () => {
  ...
  await expect(
    win.getByRole("complementary", { name: "Horizon AI" }),
  ).toBeVisible();
  await expect(
    win.getByPlaceholder("Ask anything about this page…"),
  ).toBeVisible();
...
test("right-click on a tab opens the tab context menu", async () => {
  ...
  await expect(
    win.getByRole("menuitem", { name: /Pin tab|Unpin tab/ }),
  ).toBeVisible();
  await win.keyboard.press("Escape");
});
...
test("pinning a tab moves it to the front of the bar", async () => {
  ...
  while (safety-- > 0) {
    const tab = win
      ...
      .filter({ hasNot: win.getByTestId("tab-close") })
      .first();
  ...
  await expect
    .poll(
      async () =>
        await win.getByTestId("tab").first().getAttribute("data-tab-id"),
    )
    .toBe(thirdId);
});
...
test("typing in omnibox dispatches navigation:go on Enter", async () => {
  ...
  const navigated = win.evaluate(
    () =>
      new Promise<string>((resolve) => {
        const off = window.horizonAPI.on(
          "load:started",
          (payload: { url: string }) => {
...
test("Cmd+Shift+N opens a second window with the incognito badge", async () => {
  ...
  const chromeWindow = app
    .windows()
    .find((w) => w !== win && /incognito=1/.test(w.url()));
