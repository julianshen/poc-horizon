...
describe("parseAgentPolicy", () => {
  ...
  it("rejects when version is missing or non-v1", () => {
    ...
    expect(
      parseAgentPolicy({ version: "2.0", site: "X", capabilities: {} }),
    ).toBeNull();
    expect(
      parseAgentPolicy({ version: "abc", site: "X", capabilities: {} }),
    ).toBeNull();
  });

  it("accepts v1.x.y patches", () => {
    expect(
      parseAgentPolicy({ version: "1.0", site: "X", capabilities: {} }),
    ).not.toBeNull();
    expect(
      parseAgentPolicy({ version: "1.1", site: "X", capabilities: {} }),
    ).not.toBeNull();
    expect(
      parseAgentPolicy({ version: "1.0.3", site: "X", capabilities: {} }),
    ).not.toBeNull();
  });
...
describe("humanGateForTool", () => {
  ...
  it('returns "auth_change" when args mention password', () => {
    expect(humanGateForTool("type", { text: "new-password" }, policy)).toBe(
      "auth_change",
    );
  });
  ...
  it("does not flag triggers the policy does not declare", () => {
    ...
    expect(
      humanGateForTool("navigate", { url: "https://shop.example/checkout" }, p),
    ).toBeNull();
  });
});
...
describe("humanGateForTool — extended trigger coverage", () => {
  it("matches data_export trigger when args mention export/download", () => {
    ...
    expect(
      humanGateForTool("click", { x: 1, y: 1, label: "Export my data" }, p),
    ).toBe("data_export");
    expect(
      humanGateForTool(
        "navigate",
        { url: "https://x/account/download-data" },
        p,
      ),
    ).toBe("data_export");
  });
  ...
  it("matches irreversible:* triggers when args mention destructive verbs", () => {
    ...
    expect(
      humanGateForTool("click", { x: 1, y: 1, label: "Delete account" }, p),
    ).toBe("irreversible:account_delete");
  });
  ...
  it("does NOT apply heuristics to read-only tools (no false-positive prompts)", () => {
    ...
    expect(
      humanGateForTool(
        "evaluate",
        { expression: 'document.querySelector(".stripe-logo").src' },
        p,
      ),
    ).toBeNull();
...
describe("prohibitionForTool", () => {
  it("maps dark_pattern_acceptance → dismissOverlays", () => {
    ...
    expect(prohibitionForTool("dismissOverlays", p)).toBe(
