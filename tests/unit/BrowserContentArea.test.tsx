...
class StubResizeObserver {
  ...
  observe(el: Element): void { this.observed = el; }
  unobserve(): void {}
  disconnect(): void {}
  trigger(): void {
...
(globalThis as unknown as { cancelAnimationFrame: (id: number) => void }).cancelAnimationFrame = (): void => {};
...
describe('BrowserContentArea (renderer-reported content bounds)', () => {
  ...
  it('reports its bounding rect via ui:contentBounds on mount', () => {
    ...
    expect(call!.payload).toMatchObject({ x: 12, y: 146, width: 880, height: 600 });
  });
  ...
  it('re-reports when the showAI store flag toggles', () => {
    ...
    render(<BrowserContentArea />);
    flushRaf();
    const before = api().invokes.filter((i) => i.channel === 'ui:contentBounds').length;
    act(() => useBrowserStore.setState({ showAI: true }));
    flushRaf();
    const after = api().invokes.filter((i) => i.channel === 'ui:contentBounds').length;
